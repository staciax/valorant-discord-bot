...
def json_read(filename: str, force=True) -> Dict:
    """Read json file"""
    try:
...
def get_item_by_type(Itemtype: str, uuid: str) -> Dict:
    ...
    if item_type == 'Agents': ...
    elif item_type == 'Contracts': return get_contract(uuid)
    elif item_type == 'Sprays': return get_spray(uuid)
    elif item_type == 'Gun Buddies': return get_buddie(uuid)
    elif item_type == 'Player Cards': return get_playercard(uuid)
    elif item_type == 'Skins': return get_skin(uuid)
    elif item_type == 'Skins chroma': ...
    elif item_type == 'Player titles': return get_title(uuid)
        
def get_skin(uuid: str):
    '''Get Skin data'''
    try:
        skindata = json_read('cache')
...
def get_emoji_tier(skin_uuid: str) -> discord.Emoji:
    data = json_read('cache')
    uuid = data['skins'][skin_uuid]['tier']
...
def get_emoji_tier_by_bot(skin_uuid: str, bot: commands.Bot) -> discord.Emoji:
    emoji = discord.utils.get(bot.emojis, name=get_tier_name(skin_uuid) + 'Tier')
    if emoji is None:
        return get_emoji_tier(skin_uuid)
    return emoji

def get_emoji_point_by_bot(point: Literal['ValorantPointIcon', 'RadianitePointIcon'], bot: commands.Bot) -> discord.Emoji:
...
def get_skin_list(data: Dict, language: str) -> Dict:
    '''Get skins list'''
    ...
    for uuid in offer_list:
        ...
        if skin_count == 0:
            skin1 = dict(name=name, icon=icon, price=get_skin_price(uuid), tier=get_skin_tier_icon(uuid), uuid=uuid)
        elif skin_count == 1:
            skin2 = dict(name=name, icon=icon, price=get_skin_price(uuid), tier=get_skin_tier_icon(uuid), uuid=uuid)
        elif skin_count == 2:
            skin3 = dict(name=name, icon=icon, price=get_skin_price(uuid), tier=get_skin_tier_icon(uuid), uuid=uuid)
        elif skin_count == 3:
            skin4 = dict(name=name, icon=icon, price=get_skin_price(uuid), tier=get_skin_tier_icon(uuid), uuid=uuid)
        skin_count += 1
...
def get_mission_format(data: Dict, language: str) -> Dict[str, str]:
    '''Get mission format'''
...
def get_nightmarket(offer: Dict, language: str, response: Dict) -> Dict:
...
def get_item_battlepass(type: str, uuid: str, language: str, response: Dict) -> Dict:
...
def get_btp_tier_reward(tier: int, reward: List[Dict]) -> Dict:
    '''Get tier reward'''
...
def get_btp_info(contracts: Dict, data_contracts: Dict, season_id: str, language:str) -> Dict:
    '''Get battlepass info'''
...
def get_battlepass(data: Dict, season: str, language: str, response) -> Dict:
    ...
    btp = get_btp_info(data, contracts, season_id, language)
    if btp['success']:
        tier, act, xp, reward = btp['tier'], btp['act'], btp['xp'], btp['reward']

        item_reward = get_btp_tier_reward(tier, reward)
        item = get_item_battlepass(item_reward['type'], item_reward['uuid'], language, response)
